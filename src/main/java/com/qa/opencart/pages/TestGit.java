package com.qa.opencart.pages;

public class TestGit {

	TestGit() {
		
		int i = 10;
		System.out.println("Testing GIT commands");

		//New code after merging the PR for above change

		//after seeing conflicts, kept the code from other guy and code written by me too
		System.out.print("Lets try to see after this change if we can see a conflict - "
				+ "change made in by some other guy in the team and PR merged and available in master now ");

		
		System.out.print("Lets try to see after this change if we can see a conflict - "
		+ "change made in branch09-09-2025 from Working copy");
		
		//changes made in Ankita's local - raise PR

		//More changes by someone else
	}

}
